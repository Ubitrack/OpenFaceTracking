///////////////////////////////////////////////////////////////////////////////
// Ported from C# to C++
// Original file: OpenFace\lib\local\CppInerop\LandmarkDetectorInterop.h
//
...
namespace LandmarkDetectorInterop {
	...
	public:
		...
		FaceModelParameters(string & root, bool ceclm, bool clnf, bool clm)
		{
			vector<string> args;
			args.push_back(root);

			string model_loc = "model/main_ceclm_general.txt";
			if (ceclm)
			{
		...
		string & GetMTCNNLocation()
		{
			return params->mtcnn_face_detector_location;
		}

		string & GetHaarLocation()
		{
			return params->haar_face_detector_location;
		}
	...
	public:
		...
		bool DetectLandmarksInVideo(cv::Mat & rgb_image, FaceModelParameters & modelParams, cv::Mat & gray_image) {
			return ::LandmarkDetector::DetectLandmarksInVideo(rgb_image, *clnf, *modelParams.getParams(), gray_image);
		}

		bool DetectFaceLandmarksInImage(cv::Mat rgb_image, FaceModelParameters & modelParams, cv::Mat gray_image) {
			return ::LandmarkDetector::DetectLandmarksInImage(rgb_image, *clnf, *modelParams.getParams(), gray_image);
		}
			
		bool DetectFaceLandmarksInImage(cv::Mat rgb_image, cv::Rect_<float> & bounding_box, FaceModelParameters & modelParams, cv::Mat gray_image) {
